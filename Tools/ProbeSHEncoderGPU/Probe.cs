...
namespace ProbeSHEncoder
{
	public class	Probe : IComparer<Probe.Set>, IComparer<Probe.NeighborProbe>
	{
		...
		public class	Pixel
		{
			...
			public float3		Normal;					// World normal
			public float3		Albedo;					// Material albedo
			public float3		AlbedoHSL;				// Material albedo in HSL format
			public float		F0;						// Material Fresnel coefficient
			public float3		StaticLitColor;			// Color of the statically lit environment
			...
			public float3		View;					// View vector pointing to that pixel
			...
			public void			SetAlbedo( float3 _Albedo )
			{
				Albedo = _Albedo;
...
// 	S += 1e-6f;
			...
			public float	ComputeMetric( Pixel _Other, float _PositionDistanceWeight, float _NormalDistanceWeight, float _AlbedoDistanceWeight )
			{
				...
				float	NormalDistance = 0.5f * (1.0f - _Other.Normal.Dot(  Normal ));
		...
		public class	Set : Pixel, IComparer<Pixel>
		{
			...
			public float3		Tangent = float3.Zero;
			public float3		BiTangent = float3.Zero;

			// The generated SH coefficients for this set
			public float3[]	SH = new float3[9];
			...
			public void			EncodeSH()
			{
				...
				foreach ( Pixel P in SetPixels )
				{
					...
					double	Factor  = P.SolidAngle									// Solid angle for SH weight, obvious
									* Math.Max( 0.0, P.Normal.Dot( this.Normal ) )	// This weight is to account for the fact that the point is well aligned with the set's plane the lighting was computed for
									* Math.Max( 0.0, -P.View.Dot( P.Normal ) );		// This weight is to account for the fact that the point is well aligned with the view vector
...
//				double	Normalizer = 1.0 / (4.0 * Math.PI);
				...
				for ( int i=0; i < 9; i++ )
					SH[i] = new float3( (float) (Normalizer * SHR[i]), (float) (Normalizer * SHG[i]), (float) (Normalizer * SHB[i]) );

				// Apply filtering
				FilterLanczos( SH, FILTER_WINDOW_SIZE );
			}
...
//				FilterGaussian( FILTER_WINDOW_SIZE );	// Smoothes A LOT but according to the source of filters code, it's better if using HDR light sources
				FilterHanning( SH, FILTER_WINDOW_SIZE );
			}
			...
			public void			FindPrincipalAxes()
			{
				...
				for ( int Angle=0; Angle < 180; Angle+=2 )
				{
					...
					foreach ( Pixel P in SetPixels )
					{
						...
						float			Dot = Math.Abs( Center2Pixel.Dot( OrthoDir ) );	// Gives the distance to the closest point on the rotating line
				...
				float	Diff = Tangent.Dot( BiTangent );	// We'd prefer a diff of 0, meaning the found axes are orthogonal...
...
// TODO: Choose well spaced pixels to cover the maximum area for this set!
				...
				foreach ( Pixel P in SetPixels )
				{
					...
					{
						float	Distance2Sample = (P.Position - Samples[SampleIndex].Position).LengthSquared;
			...
			{
				float	DotX = x.View.Dot( __ReferencePixel.View );
				float	DotY = y.View.Dot( __ReferencePixel.View );
		...
		public class	NeighborProbe
		{
			...
			public float3			Direction = float3.Zero;		// Direction to the probe
		...
		public float4x4[]			m_Side2World = new float4x4[6];
		...
		public float3				m_BBoxMin;
		public float3				m_BBoxMax;

		// Static and occlusion SH
		public float3[]				m_StaticSH = new float3[9];
		...
		public float3[]				m_SHSumDynamic = new float3[9];
		public float3[]				m_SHSumEmissive = new float3[9];
		...
		public void	LoadCubeMap( FileInfo _POMCubeMap )
		{
			...
			m_MinDistance = 1e6;
			m_MaxDistance = 0.0;
			m_BBoxMin = 1e38f * float3.One;
			m_BBoxMax = -1e38f * float3.One;
					...
					using ( BinaryReader R = new BinaryReader( S ) )
						for ( int Y=0; Y < CUBE_MAP_SIZE; Y++ )
							for ( int X=0; X < CUBE_MAP_SIZE; X++ )
							{
								...
								Pix.SetAlbedo( new float3( Red, Green, Blue ) );
							}
					...
					using ( BinaryReader R = new BinaryReader( S ) )
						for ( int Y=0; Y < CUBE_MAP_SIZE; Y++ )
							for ( int X=0; X < CUBE_MAP_SIZE; X++ )
							{
								...
								Pix.StaticLitColor = new float3( Red, Green, Blue );
					...
					using ( BinaryReader R = new BinaryReader( S ) )
						for ( int Y=0; Y < CUBE_MAP_SIZE; Y++ )
							for ( int X=0; X < CUBE_MAP_SIZE; X++ )
							{
								...
								float3	csView = new float3( 2.0f * (0.5f + X) / CUBE_MAP_SIZE - 1.0f, 1.0f - 2.0f * (0.5f + Y) / CUBE_MAP_SIZE, 1.0f );
								float			Distance2Texel = csView.Length;
												csView /= Distance2Texel;
								float3	wsView = (float3) (new float4( csView, 0.0f ) * m_Side2World[CubeFaceIndex]);
								...
								float3		wsPosition = new float3( Distance * wsView.x, Distance * wsView.y, Distance * wsView.z );

								Pix.Position = wsPosition;
								Pix.View = wsView;
								Pix.Normal = new float3( Nx, Ny, Nz ).Normalized;
								Pix.SolidAngle = SolidAngle;
								Pix.Importance = -wsView.Dot( Pix.Normal) / (Distance * Distance);
...
//										throw new Exception( "WTH?? Negative importance here!" );
								...
								m_BBoxMin.Min( wsPosition );
								m_BBoxMax.Max( wsPosition );
							}
...
//			double	Normalizer = 1.0 / (4.0 * Math.PI);
			...
			for ( int i=0; i < 9; i++ )
			{
				m_StaticSH[i] = new float3( (float) (Normalizer * SHR[i]), (float) (Normalizer * SHG[i]), (float) (Normalizer * SHB[i]) );
...
// 	Console.WriteLine( "GOTCHA!" );
		...
		private bool	CheckAndAcceptPixel( Set _S, Pixel _PreviousPixel, Pixel _P, List<Pixel> _SetRejectedPixels )
		{
			...
			else
			{
				...
				float	Dot = _PreviousPixel.Normal.Dot( _P.Normal );
				if ( Dot > ANGULAR_THRESHOLD )
				{
					...
					float	ToleranceFactor = -_P.Normal.Dot( _P.View );						// Weight by the surface's slope to be more tolerant for slant surfaces
...
// 						}
		...
		private void	PrepareCubeMapFaceTransforms()
		{
			...
			for ( int CubeFaceIndex=0; CubeFaceIndex < 6; CubeFaceIndex++ )
			{
				float4x4	Camera2Local = new float4x4();
				Camera2Local.r0.Set( SideRight[CubeFaceIndex], 0 );
				Camera2Local.r1.Set( SideAt[CubeFaceIndex].Cross( SideRight[CubeFaceIndex] ), 0 );
				Camera2Local.r2.Set( SideAt[CubeFaceIndex], 0 );
				Camera2Local.r3.Set( float3.Zero, 1 );
