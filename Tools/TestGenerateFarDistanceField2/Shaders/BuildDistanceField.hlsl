...
void	CS_X( uint3 _GroupID : SV_GROUPID, uint3 _GroupThreadID : SV_GROUPTHREADID, uint3 _DispatchThreadID : SV_DISPATCHTHREADID ) {
	...
	float4	projPosition = float4( 2.0 * (PixelPosition.x + 0.5) / Dimensions.x - 1.0, 1.0 - 2.0 * (PixelPosition.y + 0.5) / Dimensions.y, 0.0, 1.0 );
	...
	float	Distance0 = _TexSource.SampleLevel( PointClamp, UVW0, 0.0 ).w + 1 + PixelOffset;
	float	Distance1 = _TexSource.SampleLevel( PointClamp, UVW1, 0.0 ).w;
