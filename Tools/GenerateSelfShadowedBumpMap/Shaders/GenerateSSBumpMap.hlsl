...
//	_Dir.z = max( 1.0 / MAX_STEPS_COUNT, _Dir.z );	// Can't take more than maximum steps to exit the surface
	...
	uint	StepIndex = 0;
	while ( Position.z < 1.0 && Occlusion > 1e-3 && StepIndex < _MaxStepsCount )
	{
		Position += _Dir;
		...
		Occlusion *= 1.0 - max( 0.0, _AOFactor * (H - Position.z) );	// Will get darker as soon as height map goes above ray position
...
void	CS( uint3 _GroupID : SV_GROUPID, uint3 _GroupThreadID : SV_GROUPTHREADID )
{
	...
	if ( RayIndex == 0 )
	{
		_Target[PixelPosition] = (gs_Occlusion[0] + gs_Occlusion[1] + gs_Occlusion[2] + gs_Occlusion[3]) / _RaysCount;
