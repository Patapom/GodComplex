...
//
groupshared float4	gs_csPositions[1+2*64];
[numthreads( 64, 1, 1 )]
void	CS( uint3 _GroupID : SV_GROUPID, uint3 _GroupThreadID : SV_GROUPTHREADID, uint3 _DispatchThreadID : SV_DISPATCHTHREADID ) {
	...
	float3	UVW = (0.5 + float3( PixelPosition )) / Dimensions;

	gs_csPositions[1+PixelOffset] = _TexSource.SampleLevel( PointClamp, UVW );
