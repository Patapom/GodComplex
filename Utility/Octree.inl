...
template<typename T> void	Octree<T>::Append( const float3& _Position, float _Radius, T _Value )
{
	...
	m_pROOT->Append( NewContent, m_Min, m_Size, 0 );
}
...
template<typename T> void	Octree<T>::Node::Append( const Content& _Content, const float3& _Min, float _Size, U32 _Level )
{
	...
	for ( U32 Z=ZStart; Z < ZEnd; Z++, CellMin.z=CellCenter.z )
	{
		...
		for ( U32 Y=YStart; Y < YEnd; Y++, CellMin.y=CellCenter.y )
		{
			...
			{
				GetChildNode( X, Y, Z ).Append( _Content, CellMin, HalfSize, _Level+1 );
			}
		}
	}
}

template<typename T> void	Octree<T>::Node::Fetch( const float3& _Position, List<T>& _Result, const float3& _Min, float _Size )
{
...
template<typename T> typename Octree<T>::Node&	Octree<T>::Node::GetChildNode( U32 _X, U32 _Y, U32 _Z )
{
