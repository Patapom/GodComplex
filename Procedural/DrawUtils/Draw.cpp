...
void	DrawUtils::DrawLine( float x0, float y0, float x1, float y1, float thickness, FillDelegate _Filler, void* _pData ) const
{
	...
	float4	pVertices[4];
	pVertices[0] = float4( P0 + thickness * (V - U), 0, 1 );
	pVertices[1] = float4( P1 + thickness * (V + U), 1, 1 );
	pVertices[2] = float4( P1 + thickness * (U - V), 1, 0 );
	pVertices[3] = float4( P0 - thickness * (U + V), 0, 0 );
...
//////////////////////////////////////////////////////////////////////////
// Scratches
void	DrawUtils::DrawScratch( const float2& _Position, const float2& _Direction, float _Length, float _ThicknessStart, float _ThicknessEnd, float _CurveAngle, float _StepSize, ScratchFillDelegate _Filler, void* _pData ) const
{
	m_Infos.pData = _pData;
	...
	pVertices[2] = float4( Position + Thickness * Normal, 0, 0 );
...
//////////////////////////////////////////////////////////////////////////
void	DrawUtils::Transform( const float4& _SourcePosition, float4& _TransformedPosition ) const
{
...
void	DrawUtils::DrawQuad( float4 _pVertices[], DrawContext& _Context ) const
{
