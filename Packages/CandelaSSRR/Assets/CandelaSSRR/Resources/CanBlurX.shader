...
{	
	...
	//X-Blur
		  ...
		  p1 += tex2Dlod( _MainTex, float4(i.uv + float2(-_MainTex_TexelSize.x,   0)*blurRad,0,0))
		   	  + tex2Dlod( _MainTex, float4(i.uv + float2(-_MainTex_TexelSize.x*2, 0)*blurRad,0,0))
		      + tex2Dlod( _MainTex, float4(i.uv + float2(-_MainTex_TexelSize.x*3, 0)*blurRad,0,0))
		   	  + tex2Dlod( _MainTex, float4(i.uv + float2(+_MainTex_TexelSize.x,   0)*blurRad,0,0))
		   	  + tex2Dlod( _MainTex, float4(i.uv + float2(+_MainTex_TexelSize.x*2, 0)*blurRad,0,0))
		   	  + tex2Dlod( _MainTex, float4(i.uv + float2(+_MainTex_TexelSize.x*3, 0)*blurRad,0,0));
