...
//   }
  ...
  public class Typesetter<TFont, TGlyph> where TFont: IFont<TGlyph> {
    ...
    public static ListDisplay<TFont, TGlyph> CreateLine( AtomsList list, TFont font, TypesettingContext<TFont, TGlyph> context, LineStyle style ) {
    ...
    private static TGlyph _FindVariantGlyph(FontMathTable<TFont, TGlyph> mathTable, IGlyphBoundsProvider<TFont, TGlyph> boundsProvider, TFont styleFont, TGlyph rawGlyph, float targetWidth, out float glyphAscent, out float glyphDescent, out float glyphWidth) {
      ...
      var boundingBoxes = boundsProvider.GetBoundingRectsForGlyphs(styleFont, glyphs.AsForEach(), nGlyphs);
      var (advances, _) = boundsProvider.GetAdvancesForGlyphs(styleFont, glyphs.AsForEach(), nGlyphs);
      TGlyph currentGlyph = null;
    ...
    private static List<IMathAtom> _PreprocessMathList(IMathList list, TypesettingContext<TFont, TGlyph> context) {
      ...
      foreach (IMathAtom atom in list.Atoms) {
        ...
        if (atom.AtomType == MathAtomType.Ordinary || atom.AtomType == MathAtomType.UnaryOperator) {
          ...
          if (prevNode != null && prevNode.AtomType == MathAtomType.Ordinary && prevNode.Superscript == null && prevNode.Subscript == null) {
            prevNode.Fuse(atom);
    ...
    private TGlyph _FindGlyph(TGlyph rawGlyph, float height,
      ...
      var rects = _context.GlyphBoundsProvider.GetBoundingRectsForGlyphs(_styleFont, variants.AsForEach(), nVariants);
      var advances = _context.GlyphBoundsProvider.GetAdvancesForGlyphs(_styleFont, variants.AsForEach(), nVariants).Advances;
