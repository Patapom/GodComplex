...
namespace WMath
{
    ...
    public class Vector
	{
		...
		// Constructors
		public						Vector()										{}
		public						Vector( Vector2D _Source )						{ Set( _Source ); }
		public						Vector( Vector _Source )						{ Set( _Source ); }
		public						Vector( Vector4D _Source )						{ Set( _Source ); }
		public						Vector( Point _Source )							{ Set( _Source ); }
		public						Vector( float _x, float _y, float _z )			{ Set( _x, _y, _z ); }
		...
		public float				LengthSquare									{ get { return x * x + y * y + z * z; } }
		...
		public static Vector		Parse( string _Source )
		{
			string[]	Members = _Source.Split( new System.Char[] { ';' } );
			if ( Members.Length != 3 )
				return	null;

			return	new Vector( float.Parse( Members[0] ),
		...
		// Cast operators
		public static explicit		operator Vector2D( Vector _Source )				{ return new Vector2D( _Source ); }
		public static explicit		operator Vector4D( Vector _Source )				{ return new Vector4D( _Source ); }
		public static explicit		operator Point( Vector _Source )				{ return new Point( _Source ); }
		...
		public static Vector		operator-( Vector _Op0 )						{ return new Vector( -_Op0.x, -_Op0.y, -_Op0.z ); }
		public static Vector		operator+( Vector _Op0 )						{ return new Vector( +_Op0.x, +_Op0.y, +_Op0.z ); }
		public static Vector		operator+( Vector _Op0, Vector _Op1 )			{ return new Vector( _Op0.x + _Op1.x, _Op0.y + _Op1.y, _Op0.z + _Op1.z ); }
		public static Vector		operator-( Vector _Op0, Vector _Op1 )			{ return new Vector( _Op0.x - _Op1.x, _Op0.y - _Op1.y, _Op0.z - _Op1.z ); }
		public static Vector		operator*( Vector _Op0, Vector _Op1 )			{ return new Vector( _Op0.x * _Op1.x, _Op0.y * _Op1.y, _Op0.z * _Op1.z ); }
		public static Vector		operator*( Vector _Op0, float _s )				{ return new Vector( _Op0.x * _s, _Op0.y * _s, _Op0.z * _s ); }
		public static Vector		operator*( float _s, Vector _Op0 )				{ return new Vector( _Op0.x * _s, _Op0.y * _s, _Op0.z * _s ); }
		public static Vector		operator*( Vector _Op0, Matrix3x3 _Op1 )
		{
		...
		public static Vector		operator^( Vector _Op0, Vector _Op1 )			{ return new Vector( _Op0.y * _Op1.z - _Op0.z * _Op1.y, _Op0.z * _Op1.x - _Op0.x * _Op1.z, _Op0.x * _Op1.y - _Op0.y * _Op1.x ); }

		// Logic operators
		public static bool			operator==( Vector _Op0, Vector _Op1 )
		{
		...
		public static bool			operator<( Vector _Op0, Vector _Op1 )			{ return _Op0.x < _Op1.x && _Op0.y < _Op1.y && _Op0.z < _Op1.z; }
		public static bool			operator<=( Vector _Op0, Vector _Op1 )			{ return _Op0.x < _Op1.x + float.Epsilon && _Op0.y < _Op1.y + float.Epsilon && _Op0.z < _Op1.z + float.Epsilon; }
		public static bool			operator>( Vector _Op0, Vector _Op1 )			{ return _Op0.x > _Op1.x && _Op0.y > _Op1.y && _Op0.z > _Op1.z; }
		public static bool			operator>=( Vector _Op0, Vector _Op1 )			{ return _Op0.x > _Op1.x - float.Epsilon && _Op0.y > _Op1.y - float.Epsilon && _Op0.z > _Op1.z - float.Epsilon; }
	...
	public class VectorTypeConverter : System.ComponentModel.TypeConverter
	{
		...
		{
			if ( _Value is string )
				return	Vector.Parse( _Value as string );
		...
		{
			if ( _DestinationType == typeof(string) )
				return	(_Value as Vector).ToString();
		...
		{
			return	System.ComponentModel.TypeDescriptor.GetProperties( typeof(Vector), new System.Attribute[] { new System.ComponentModel.BrowsableAttribute( true ) } );
		}
	}
}
