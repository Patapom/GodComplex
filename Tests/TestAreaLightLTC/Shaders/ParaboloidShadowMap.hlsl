...
//_wsPosition += 0.1 * _wsNormal;
	...
	// Sample exp( -c.z )
	float	Exp_BlockerDistance = 1.0 - _TexShadowMap.SampleLevel( LinearClamp, UV, 0.0 );		// Retrieve the original exp( -EXP_CONSTANT * Distance/SHADOW_ZFAR ) since we stored its complement
