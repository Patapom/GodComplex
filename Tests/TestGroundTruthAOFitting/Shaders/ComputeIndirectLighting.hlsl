...
cbuffer	CBInput : register( b0 ) {
	uint2	_Dimensions;		// Texture dimensions
...
RWTexture2D<float>			_targetIllumlinance : register( u0 );

Texture2D<float>			_sourceAO : register( t1 );
Texture2D<uint2>			_sourceOffsetCount : register( t2 );
Texture2D<float>			_sourceHeight : register( t3 );
Texture2D<float3>			_sourceNormals : register( t4 );
StructuredBuffer<uint>		_indirectPixelsStack : register( t5 );


groupshared uint2			gs_OffsetCount;
groupshared float			gs_position_mm;
groupshared float			gs_normal;
groupshared float			gs_accumulator[MAX_THREADS];
...
void	CS( uint3 _groupID : SV_groupID, uint3 _groupThreadID : SV_groupThreadID ) {
	...
	if ( rayIndex == 0 ) {
		gs_OffsetCount = _sourceOffsetCount[pixelPosition];
		gs_position_mm = float3( _TexelSize_mm * pixelPosition, _Displacement_mm * _sourceHeight[pixelPosition] );
		gs_normal = _sourceNormals.SampleLevel( LinearClamp, float2( pixelPosition ) / _Dimensions, 0 );
	}

	GroupMemoryBarrierWithGroupSync();

	if ( rayIndex >= gs_OffsetCount.y )
		return;	// Uselesss thread...

	// Retrieve indirect sample's position and normal
	uint	packedNeighborPixelPosition = _indirectPixelsStack[gs_OffsetCount.x + rayIndex];
	uint2	neighborPixelPosition = uint2( packedNeighborPixelPosition & 0xFFFFU, packedNeighborPixelPosition >> 16 );
	uint2	tiledNeighborPixelPosition = neighborPixelPosition % _Dimensions;
...
//	float3	neighborNormal = _sourceNormals.SampleLevel( LinearClamp, neighborUV, 0 );
	...
	float	directNeighborIlluminance = PI * sinAlpha * sinAlpha;				// Assuming a unit luminance in all directions from normal cone
	float	directNeighborLuminance = (1.0 / PI) * directNeighborIlluminance;	// Assuming a unit albedo (will be scaled on CPU by actual albedo)

	// We then accumulate the neighbor's luminance weighted by the dot product
	float3	incomingDirection = neighborPosition_mm - gs_position_mm;
	float	distance2Neighbor = length( incomingDirection );
			incomingDirection /= distance2Neighbor;

	float	cosTheta = saturate( dot( incomingDirection, gs_normal ) );
	float	sinTheta = sqrt( 1.0 - cosTheta*cosTheta );

	gs_accumulator[rayIndex] += directNeighborLuminance * cosTheta * sinTheta * dTheta;	// C'est quoi dTHeta???
	...
	if ( rayIndex == 0 ) {
		float	result = 0.0;
		for ( uint i=0; i < gs_OffsetCount.y; i++ )
