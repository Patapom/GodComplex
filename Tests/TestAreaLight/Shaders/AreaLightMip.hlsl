...
Texture3D< float4 >	_TexAreaLightMIP : register(t0);


// Samples the area light 3D texture
float4	SampleAreaLight( float2 _UV0, float2 _UV1 ) {
	...
	float	BorderSize = max( RadiusPixel.x, RadiusPixel.y );
	float	MipLevel = log2( 1e-3 + BorderSize );

	// Compute the UVs for that specific "mip level"
	float2	BorderSizeUV = RadiusPixel * _AreaLightTexDimensions.zw;
	float	UV_topleft = BorderSizeUV;
	float	UV_bottomright = 1.0 - BorderSizeUV;

	// Compute remapped UV
	float2	UV = 0.5 * (_UV0 + _UV1);
			UV = lerp( UV_topleft, UV_bottomright, UV );

	// Compute Z from mip
	float	Z = MipLevel / 9.0;

	return _TexAreaLightMIP.SampleLevel( LinearBorder, float3( UV, Z ), 0.0 );
}
