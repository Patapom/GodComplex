...
// We assume there exists such methods declared somewhere where _pixelPosition is the position in screen space
...
float3	SampleIrradiance( float2 _ssPosition, float _H0, float _radius, float2 _integralFactors, inout float _maxCos ) {
	...
	float	deltaH = FetchDepth( _ssPosition ) - _H0;
...
//	_radialStepSize, size of a radial step to jump from one sample to another
...
//
float3	GatherIrradiance( float2 _ssPosition, float2 _ssDirection, float _Z0, float3 _csNormal, float _radialStepSize, uint _stepsCount, out float3 _ssBentNormal, out float2 _coneAngles, inout float4 _DEBUG ) {
