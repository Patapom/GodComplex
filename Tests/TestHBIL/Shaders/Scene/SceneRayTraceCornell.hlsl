...
//static const float3	LIGHT_FLUX = 2500.0;
static const float3	LIGHT_FLUX = 1500.0;
static const float3	LIGHT_REFLECTANCE = 0.78;
static const float3	LIGHT_SIZE = float3( 1.0, 0.0, 1.0 );

// Emissive
//static const float3	CORNELL_EMISSIVE_RECT_POS = float3( 4.6, 0.6, 1.6 ) - 0.5 * CORNELL_SIZE;
//static const float3	CORNELL_EMISSIVE_RECT_SIZE = float3( 1.0, 1.0, 0.1 );	// It's a thin rectangle
//static const float	CORNELL_EMISSIVE_RECT_ANGLE = 0.29145679447786709199560462143289;	// ~16°

static const float3	CORNELL_EMISSIVE_RECT_POS = float3( 1.6, 0.6, 4.6 ) - 0.5 * CORNELL_SIZE;
static const float3	CORNELL_EMISSIVE_RECT_SIZE = float3( 1.0, 1.0, 0.1 );	// It's a thin rectangle
static const float	CORNELL_EMISSIVE_RECT_ANGLE = -0.29145679447786709199560462143289;	// ~16°
//static const float3	CORNELL_EMISSIVE_RECT_LUMINANCE = 10.0 * float3( 0.2, 0.8, 1.0 );
static const float3	CORNELL_EMISSIVE_RECT_LUMINANCE = 20.0 * float3( 1.0, 0.8, 0.2 );
	...
	if ( testDistances.x < testDistances.y ) {
		...
		if ( hitSides.y == 2 ) {
			...
			float2	floorUV = 0.5 + wsFloorPos / CORNELL_SIZE.xz;
				...
				floorUV *= 2.0;
			#endif
			result.albedo = _tex_FloorAlbedo.SampleLevel( LinearWrap, floorUV, 0.0 );
			result.wsNormal = _tex_FloorNormal.SampleLevel( LinearWrap, floorUV, 0.0 ).xzy;
result.wsNormal = normalize( result.wsNormal * float3( 1, 0.5, 1 ) );	// Amplify a bit
			result.roughness = _tex_FloorRoughness.SampleLevel( LinearWrap, floorUV, 0.0 );
			...
			result.albedo = _tex_WallAlbedo.SampleLevel( LinearWrap, wallUV, 0.0 );
			result.wsNormal = _tex_WallNormal.SampleLevel( LinearWrap, wallUV, 0.0 );
			result.wsNormal = hitSides.y == 4 ? float3( -result.wsNormal.x, result.wsNormal.y, result.wsNormal.z ) : float3( -result.wsNormal.x, result.wsNormal.y, -result.wsNormal.z );
//result.wsNormal = normalize( result.wsNormal * float3( 1, 0.5, 1 ) );	// Amplify a bit
			result.roughness = _tex_WallRoughness.SampleLevel( LinearWrap, wallUV, 0.0 );
		}
	}
...
//		result.wsNormal = Side2Normal( hitSides.x );
	...
	switch ( uint(result.materialID) ) {
		case MAT_ROOM :				result.albedo = CORNELL_ROOM_REFLECTANCE; result.roughness = 0.9; break;
		...
		case MAT_SMALL_BOX :		result.albedo = CORNELL_SMALL_BOX_REFLECTANCE; result.roughness = 0.4; break;
		case MAT_LARGE_BOX :		result.albedo = CORNELL_LARGE_BOX_REFLECTANCE; result.roughness = 0.4; break;
		case MAT_EMISSIVE :			result.emissive = CORNELL_EMISSIVE_RECT_LUMINANCE; result.roughness = 0.1; break;
	}
...
LightingResult	LightScene( float3 _wsPosition, float3 _wsNormal, float3 _wsView, float _roughness, float3 _IOR, float _pixelSize_m, float3 _wsBentNormal, float2 _cosConeAnglesMinMax, float _noise ) {
	...
	LightInfoPoint	lightInfo;
					...
					lightInfo.flux = _sunIntensity * LIGHT_FLUX * GetShadowPoint( _wsPosition, _wsNormal, _pixelSize_m, lightInfo.wsPosition, lightInfo.distanceAttenuation.y, _tex_ShadowMap, _noise, SHADOW_BIAS );
