...
#define SAMPLE_TERRAIN	1	// Define for nice terrain

#if	SAMPLE_TERRAIN
	#define	STEPS_COUNT	20		// Terrain is super expensive!
#else
...
Intersection	TraceScene( float3 _wsPos, float3 _wsDir ) {
	...
	result.shade = 0;
	result.wsHitPosition = float4( _wsPos, 1e6 );
	result.wsNormal = float3( 0, 1, 0 );
	...
	if ( testDistances.x < testDistances.y ) {
		...
		float3	lsNormal = 0;
		float	lsHitDistance = ComputeHeightFieldIntersection( lsPosIn, lsDir, L, STEPS_COUNT, lsNormal );
		if ( lsHitDistance < 1e5 ) {
			float3	lsHitPos = lsPosIn + lsHitDistance * lsDir;
			result.wsHitPosition.xyz = BBOX_MIN + lsHitPos * 2.0 * BOX_HALF_SIZE;
			result.wsHitPosition.w = length( result.wsHitPosition.xyz - _wsPos );
			result.wsNormal = float3( lsNormal.x, lsNormal.z, -lsNormal.y );
...
LightingResult	LightScene( float3 _wsPosition, float3 _wsNormal, float3 _wsView, float _roughness, float3 _IOR, float _pixelSize_m, float3 _wsBentNormal, float2 _cosConeAnglesMinMax, float _noise ) {
	...
	float3	SUN_COLOR = lerp( float3( 1, 0.2, 0.05 ), float3( 1, 1, 1 ), pow3( saturate( -_directionalShadowMap2World[2].y ) ) );

	LightInfoDirectional	lightInfo;
							lightInfo.illuminance = _sunIntensity * SUN_COLOR * GetShadowDirectional( _wsPosition, _wsNormal, _pixelSize_m, _tex_ShadowMapDirectional, _noise, SHADOW_BIAS );
