...
#else
	// Read a single sample per-pixel... Ugly pattern but we expect it to be alleviated by temporal filtering
	// If this option is chosen then the recomposition is useless and we can directly sample the split buffers during scene lighting!
	float4	targetRadiance[4*4];
	float4	targetBentCone[4*4];

	for ( uint index0=0; index0 < 4*4; index0++ ) {
		sourcePixelIndex.z = index0;
		float3	radiance = _tex_splitIrradiance[sourcePixelIndex].xyz;
		float4	bentCone = _tex_splitBentCone[sourcePixelIndex];

		// Use AO to compute cone angle
		float	sumAO = bentCone.w;
				sumAO *= sumAO;	// Was stored as sqrt(AO) to favor dark values
		float	cosAverageConeAngle = 1.0 - sumAO;
		float3	csAverageBentNormal = normalize( bentCone.xyz );
		float	stdDeviation = 0.0;
		...
		targetRadiance[index0] = float4( radiance, 0 );
		targetBentCone[index0] = csBentCone;
	}
#endif
