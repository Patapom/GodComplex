...
void	CS_Copy( uint3 _groupID : SV_groupID, uint3 _groupThreadID : SV_groupThreadID, uint3 _dispatchThreadID : SV_dispatchThreadID ) {
	...
	float	Zproj = _tex_sourceDepth[_dispatchThreadID.xy];
	float	Q = _ZNearFar_Q_Z.z;
	float	Z = (-Q * _ZNearFar_Q_Z.x / (Zproj - Q));

	_tex_targetDepth[_dispatchThreadID.xy] = Z / Z_FAR;
}
